...
def calculate_demographic_data(print_data=True):
    ...
    country_counts = df.groupby('native-country')['salary'].apply(lambda x: (x == ' >50K').sum() / len(x) * 100).sort_values(ascending=False)
    
    highest_earning_country = country_counts.index[0].strip()
    highest_earning_country_percentage = round(country_counts.iloc[0], 1)

    # Identify the most popular occupation for those who earn >50K in India.
    india_rich = df[(df['native-country'] == ' India') & (df['salary'] == ' >50K')]
    top_IN_occupation = india_rich['occupation'].value_counts().index[0].strip()
